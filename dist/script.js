...
form.addEventListener('submit', (e) => {
  ...
  chatroomSocket.emit('send', text)
})


chatroomSocket.emit('new-user', username);
chatroomSocket.on('user-joined', (name) => {
...
chatroomSocket.on('user-numbers', (user_num) => {
  document.getElementById('users-number').innerText = `${user_num} users online`
})
chatroomSocket.on('recieve', (data) => {
  const messageBox = document.createElement('div')
...
chatroomSocket.on('user-left', (name) => {
...
function typingStatus(isTyping) {
  chatroomSocket.emit('typing-status', isTyping);
}
...
chatroomSocket.on('show-typing-status', (data) => {
